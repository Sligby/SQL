...
FROM apps
WHERE rating < 3 AND price = (SELECT MAX(price) FROM apps)
LIMIT 1;
...
FROM apps
WHERE last_updated = (SELECT MIN(last_updated) FROM apps);

-- Find the most expensive app (the query is very similar to #11).
SELECT app_name, price
FROM apps
WHERE price = (SELECT MAX(price) FROM apps);
...
SELECT app_name, reviews, min_installs, CAST(min_installs AS FLOAT)/reviews AS proportion
FROM apps
